...
func (s ParcelStore) Get(number int) (Parcel, error) {
	...
	p := Parcel{}
	row := s.db.QueryRow("SELECT * FROM parcel WHERE id = :id", sql.Named("id", number))
...
func (s ParcelStore) SetAddress(number int, address string) error {
	...
	if status != ParcelStatusRegistered {
		err := errors.New("Адрес не может быть изменен, посылка уже отправлена!")
		return err
	}
	_, err = s.db.Exec("UPDATE parcel SET addredd = :address WHERE number = :number",
...
func (s ParcelStore) Delete(number int) error {
	...
	if status != ParcelStatusRegistered {
		err := errors.New("Запись не может быть удалена, посылка уже отправлена!")
		return err
	}
